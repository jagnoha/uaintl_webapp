...
const ProductForm = (props) => {
  ...
  const handleChange = (e, { name, value }) => {
    try {
      ...
      /*e.target.value && e.target.value.length > 0
        ? props.validatePublishButton(true)
        : props.validatePublishButton(false);*/

      props.validatePublishButton(
        validateProduct({ ...product, [name]: value }, ebayProduct)
      );

      props.validateUpdateButton(
        validateProduct({ ...product, [name]: value }, ebayProduct)
      );
    } catch (error) {
      console.log(error);
  ...
  const validateProduct = (product, productEbay) => {
    ...
    return product &&
      ...
      productEbay['primaryCategory'].length > 0 &&
  ...
  const fetchProduct = async (item) => {
    try {
      ...
      //console.log(typeof productTemp.manufacturerPartNumbers);
      let mpn =
      ...
      //console.log(mpn.concat(ipn, opn));
      ...
      props.validatePublishButton(
        validateProduct(productTemp, ebayItemResultTemp)
      );
      props.validateUpdateButton(
        validateProduct(productTemp, ebayItemResultTemp)
      );
  ...
  const handleChangePartNumbers = async (e, { value }) => {
    try {
      ...
      props.validatePublishButton(
        validateProduct({ ...product, partNumbers: value }, ebayProduct)
      );
      props.validateUpdateButton(
        validateProduct({ ...product, partNumbers: value }, ebayProduct)
      );
  ...
  const handleChangeImages = async (imageList, addUpdateIndex) => {
    try {
      ...
      props.validatePublishButton(
        validateProduct(
          { ...product, images: JSON.stringify(newImageList) },
          ebayProduct
        )
      );
      props.validateUpdateButton(
  ...
  const handleRemoveImages = async (index) => {
    try {
      ...
      props.validatePublishButton(
        validateProduct(
          { ...product, images: JSON.stringify(newList) },
          ebayProduct
        )
      );
      props.validateUpdateButton(
    ...
    try {
      ...
      props.validatePublishButton(
        validateProduct(
          ...
          { ...ebayProduct, primaryCategory: newPrimaryCategory }
        )
      );
      props.validateUpdateButton(
        validateProduct(
  ...
  /*const onCancelDeleteListing = async () => {
    try {
    ...
    try {
            ...
            ShippingServiceOptions:
              ...
              'Pickup'
                ? {
                    ...
                    ShippingService:
                      shippingDetailsParsed.ShippingServiceOptions
                        .ShippingService,
                  }
                : [
                    {
                      FreeShipping: true,
                      ShippingServicePriority: 1,
                      ShippingService: 'FlatRateFreight',
                    },
                    {
                      FreeShipping: true,
      ...
      if (
        ...
        const productInput = {
          ...
          hasCompatibilityList:
            JSON.parse(product.inStock).length > 0 ? false : true,
          isDraft: false,
          ebayItemId: itemID,
          price: product.price,
        };
  ...
  const saveListingDraft = async () => {
    try {
      ...
      toast(successMessage(`Listing "${product.title}" updated!`));
      console.log('Saving a File!');
      return mutationProductResult; /*{
        result: mutationProductResult,
        message: `Listing "${product.title}" updated!`,
      };*/
    } catch (error) {
      console.log(error);
      toast(getError(error));
    ...
    try {
      ...
      if (
        ...
        /*toast(
          getEbayError(
            JSON.stringify(
              res.data.result.error.meta.Errors.filter(
                (err) => err.SeverityCode !== 'Warning'
              )
                .map((err) => err.LongMessage)
                .join(' | ')
  ...
  if (props.action === 'save') {
    //let status = await saveListingDraft();
    props.closeForm(product, saveListingDraft());
  } else if (props.action === 'publish') {
    props.closeForm(product, publishingListingDraft());
  } else if (props.action === 'update') {
    props.closeForm(product, updateListingOnline());
  } else if (props.action === 'delist') {
    props.closeForm(product, delistListingOnline());
  } else if (props.action === 'delete') {
    props.closeForm(product, deleteListing());
  }
    ...
    <div style={divStyle}>
      ...
      {/*openFormDeleteListing ? (
        <Confirm
          ...
          onCancel={onCancelDeleteListing}
      ...
      <Form>
        ...
        <Form.Group>
          <Form.Input
            ...
            error={product.title.length === 0 ? "Title can't be empty" : false}
            width={14}
          />
